...
export function CommandPalette() {
  ...
  return (
    <div className="fixed inset-0 z-[99990] flex items-start justify-center pt-[15vh] px-4">
      ...
      <div
        ref={panelRef}
        className="relative w-full max-w-xl bg-[#fcfcfc] dark:bg-[#1e1508] border border-[#d4c9b0] dark:border-[#3a3020] rounded-2xl shadow-2xl overflow-hidden"
        style={{ opacity: 0 }}
      >
        {/* Input */}
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-[#e0d9cc] dark:border-[#2e2818]">
          <Search size={16} className="text-[#ad9a6f] dark:text-[#c4a97a] flex-shrink-0" />
          <input
            ref={inputRef}
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Search spaces, docs…"
            className="flex-1 bg-transparent text-[#615050] dark:text-[#f5f0e8] placeholder-[#b0a088] dark:placeholder-[#4a3d2a] outline-none text-sm"
          />
          <kbd className="text-[10px] font-mono bg-[#f0ebe0] dark:bg-[#2e2410] px-1.5 py-0.5 rounded text-[#776a6a] dark:text-[#9a8870]">ESC</kbd>
        </div>

        {/* Results */}
        <div className="max-h-80 overflow-y-auto py-2">
          {!loaded && address && (
            <p className="px-4 py-3 text-sm text-[#ad9a6f] dark:text-[#c4a97a] italic">Loading…</p>
          )}
          {loaded && results.length === 0 && (
            <p className="px-4 py-6 text-sm text-[#ad9a6f] dark:text-[#c4a97a] text-center">No results for &ldquo;{query}&rdquo;</p>
          )}
          {results.map((r, i) => (
            <button
              key={r.id}
            ...
            >
              <div className="w-7 h-7 rounded-lg bg-[#f0ebe0] dark:bg-[#2e2410] border border-[#d4c9b0] dark:border-[#3a3020] flex items-center justify-center flex-shrink-0">
                {r.icon}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm text-[#615050] dark:text-[#f5f0e8] font-medium truncate">{r.label}</p>
                {r.sub && <p className="text-[11px] text-[#ad9a6f] dark:text-[#c4a97a] font-mono">{r.sub}</p>}
              </div>
              <ArrowRight size={12} className="text-[#8a7a6a] dark:text-[#6a5f52] flex-shrink-0" />
            </button>
          ))}
        </div>

        {/* Footer */}
        <div className="border-t border-[#e0d9cc] dark:border-[#2e2818] px-4 py-2 flex items-center gap-4 text-[10px] text-[#776a6a] dark:text-[#6a5f52]">
          <span><kbd className="font-mono bg-[#f0ebe0] dark:bg-[#2e2410] px-1 rounded">↑↓</kbd> navigate</span>
          <span><kbd className="font-mono bg-[#f0ebe0] dark:bg-[#2e2410] px-1 rounded">↵</kbd> open</span>
          <span><kbd className="font-mono bg-[#f0ebe0] dark:bg-[#2e2410] px-1 rounded">esc</kbd> close</span>
        </div>
      </div>
    </div>
  );
